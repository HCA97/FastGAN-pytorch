...
if __name__ == "__main__":
    ...
    net_ig = Generator(ngf=64, nz=noise_dim, nc=3,
                       im_size=args.im_size)  # , big=args.big )
    net_ig.to(device)

    # for epoch in [10000*i for i in range(args.start_iter, args.end_iter+1)]:
    ckpt = args.ckpt  # './models/%d.pth'%(epoch)
    ...
    dist = os.path.join(os.path.split(os.path.split(ckpt)[0])[0], 'eval')
    ...
    with torch.no_grad():
        for i in tqdm(range(args.n_sample//args.batch)):
            ...
            g_imgs = F.interpolate(g_imgs, 64)
